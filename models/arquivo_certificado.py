...
class ArquivoCertificado(object):
    ...
    with ArquivoCertificado(safe_box_model, 'w') as (key, cert):
        print(key.name)
        print(cert.name)
    """

    def __init__(self, safe_box_model, method):
        self.key_fd, self.key_path = tempfile.mkstemp()
        self.cert_fd, self.cert_path = tempfile.mkstemp()

        if safe_box_model.bank_inter_cert:
            cert = base64.b64decode(safe_box_model.bank_inter_cert)
            tmp = os.fdopen(self.cert_fd, 'w')
            tmp.write(cert.decode())
            tmp.close()

        if safe_box_model.bank_inter_key:
            key = base64.b64decode(safe_box_model.bank_inter_key)
